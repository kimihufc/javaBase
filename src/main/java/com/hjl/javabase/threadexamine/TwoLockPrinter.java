...
public class TwoLockPrinter {
    ...
    public static void main(String [] agrs) throws InterruptedException {
        TwoLockPrinter printer = new TwoLockPrinter();
        // 打印A线程的锁
        ...
        // 打印a的线程
        Thread threadA = new Thread(group, printer.new Printer(lockC, lockA, 'A'));
        // 打印b的线程
        Thread threadB = new Thread(group, printer.new Printer(lockA, lockB, 'B'));
        // 打印c的线程
        Thread threadC = new Thread(group, printer.new Printer(lockB, lockC, 'C'));
        // 依次开启a b c线程
        threadA.start();
        Thread.sleep(100);
        threadB.start();
